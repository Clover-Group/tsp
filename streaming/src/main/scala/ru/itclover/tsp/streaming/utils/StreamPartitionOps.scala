...
import cats.implicits.{catsSyntaxOptionId, catsSyntaxTuple2Semigroupal, toFlatMapOps, toFoldableOps, toFunctorOps, toTraverseOps}
import fs2.Pipe


object StreamPartitionOps {
  def groupBy[F[_], A, K](selector: A => F[K])(implicit F: Concurrent[F]): Pipe[F, A, (K, fs2.Stream[F, A])] = {
    in =>

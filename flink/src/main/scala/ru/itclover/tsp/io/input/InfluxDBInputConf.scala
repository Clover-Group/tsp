...
case class InfluxDBInputConf(
  ...
  additionalTypeChecking: Option[Boolean] = Some(false),
) extends InputConf[RowWithIdx, Symbol, Any] {

  val influxConf = InfluxDBService.InfluxConf(url, dbName, userName, password, 200L, additionalTypeChecking.getOrElse(false))
}

...
import ru.itclover.tsp.io.input.{InputConf, JDBCInputConf, RawPattern}
import ru.itclover.tsp.io.output.{EventSchema, RowSchema}
import ru.itclover.tsp.DataStreamUtils.DataStreamOps
import ru.itclover.tsp.dsl.SyntaxParser
import ru.itclover.tsp.phases.NumericPhases.{SymbolExtractor, SymbolNumberExtractor}
...
class PatternsSearchStages {
  ...
  protected def getPhaseCompiler(code: String, timestampField: Symbol, fieldIndexesMap: Map[Symbol, Int])
                                (implicit timeExtractor: TimeExtractor[Row],
                                 numberExtractor: SymbolNumberExtractor[Row]): ClassLoader => Pattern[Row, Any, Any] = {
    classLoader: ClassLoader =>
      val evaluator = new EvalUtils.Eval(classLoader)
      evaluator.apply[(Pattern[Row, Any, Any])](
...
object PatternsSearchStagesDSL extends PatternsSearchStages {
  ...
  ClassLoader => Pattern[Row, Any, Any] =

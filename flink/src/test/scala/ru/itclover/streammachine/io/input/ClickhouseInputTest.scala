...
class ClickhouseInputTest extends FunSuite with Matchers {
  ...
  test("ClickhouseInput.getTypeInformation correct") {
    ...
    val typeInfoEither = ClickhouseInput.queryTypeInformation(wid_dt_tin1__conf)
    ...
    refTypes.keys should contain allElementsOf typeInfo.keys
    refTypes.values should contain allElementsOf typeInfo.values
  }
  ...
  test("ClickhouseInput.getTypeInformation invalid") {
    ...
    val invalidUrlTypeInfoEither = ClickhouseInput.queryTypeInformation(invalidUrlConf)

...
object PhaseBuilder {
  ...
  def postProcess[Event](parser: PhaseParser[Event, _, _], maxPhase: Long, asContinuous: Boolean = false)(
    ...
    parser match {
      ...
      // TODO Fix warning
      case bnp: BinaryNumericParser[Event, _, _, Double] =>
      ...
      case aph: AccumPhase[Event, _, _, _] =>
        val withProcessedInners = new AccumPhase[Event, aph.Inner, aph.AccumOutput, aph.Output](
          postProcess(aph.innerPhase, maxPhase, asContinuous = true)
            .asInstanceOf[PhaseParser[Event, aph.Inner, aph.AccumOutput]],
          aph.window,
          aph.accumulator
  ...
  def fields[Event](x: PhaseParser[Event, _, _]): List[String] = {
    x match {
      ...
      case r: Reduce[Event, _] =>
        fields(r.firstPhase) ++ r.otherPhases.foldLeft(List[String]())(

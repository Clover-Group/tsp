      ...
      with HttpService with ForAllTestContainer
{
  ...
  val typeCastingInputConf = inputConf.copy(
    query="""select *, speed as "speed(1)(2)" from SM_typeCasting_wide""")
  ...
  "Basic assertions and forwarded fields" should "work for wide dense table" in {
    ...
    Post("/streamJob/from-influxdb/to-jdbc/?run_async=0", FindPatternsRequest("1", inputConf, outputConf, basicAssertions)) ~>
        route ~> check {
  ...
  "Types casting" should "work for wide dense table" in {
    Post("/streamJob/from-influxdb/to-jdbc/?run_async=0", FindPatternsRequest("2", typeCastingInputConf, outputConf, typesCasting)) ~>
        route ~> check {
      status shouldEqual StatusCodes.OK

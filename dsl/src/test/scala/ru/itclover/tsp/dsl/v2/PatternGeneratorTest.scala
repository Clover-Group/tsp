...
class PatternGeneratorTest extends FlatSpec with Matchers with PropertyChecks {
  ...
  "Pattern generator" should "build valid patterns" in {
    gen.build("doubleSensor1 > 0 for 30 sec", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("doubleSensor1 > 0 or longSensor = 0", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("doubleSensor1 > 0 and intSensor = 0", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("(doubleSensor1 + intSensor as float64) >= 10", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("(doubleSensor1 + intSensor as float64) >= 10", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("avgOf(doubleSensor1, doubleSensor2) >= 10 for 5 min >= 100 ms", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("sin(doubleSensor1) >= 0.5 for 5 min andThen intSensor > 42", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
  ...
  "Casts" should "be performed" in {
    gen.build("doubleSensor1 as int32 > 0", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("doubleSensor1 as int64 > 0", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
    gen.build("doubleSensor1 as boolean = true", 0.0, fieldsClasses).right.value shouldBe a[(Pattern[TestEvent, _, _], PatternMetadata)]
  }
}

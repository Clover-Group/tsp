...
import ru.itclover.tsp.core.aggregators.{GroupPattern, PreviousValue, SkipPattern, TimerPattern, TimestampsAdderPattern, WindowStatistic}
...
class Optimizer[E: IdxExtractor: TimeExtractor]() extends Serializable {
  ...
  private def optimizeInners[T]: OptimizeRule[T] = {
    ...
    case SkipPattern(inner, window) if optimizable(inner)      => SkipPattern(forceState(optimizePat(inner)), window)
  }

...
//
//import ru.itclover.tsp.core.Aggregators.Timer
//import ru.itclover.tsp.core.AliasedParser.Aliased
//import ru.itclover.tsp.core._
//import ru.itclover.tsp.core.PatternResult.{Failure, Stay, Success}
//import ru.itclover.tsp.core.Time.TimeExtractor
//import ru.itclover.tsp.vectorState.VectorPhaseParser.{And, AndThen, Or, Packed}
//
...
//  */
//trait VectorPhaseParser[Event, State, +T] extends ((Event, Packed[State]) => (PatternResult[T], Packed[State])) {
...
//
//  implicit class PatternRich[Event, State, T](val parser: VectorPhaseParser[Event, State, T]) extends AnyVal {
//
//    /**
...
//      * Alias for `and`
//      */
//    def &[RightState, RightOut](rightParser: Pattern[Event, RightState, RightOut]):
...
//      * @return new AndParser
//      */
//    def lazyAnd[RightState, RightOut](rightParser: Pattern[Event, RightState, RightOut]):
...
//      * @return new OrParser
//      */
//    def or[RightState, RightOut](rightParser: Pattern[Event, RightState, RightOut]):
...
//
//    def until[State2](condition: Pattern[Event, State2, Boolean]) = UntilParser(this, condition)
//
...
//
//  override def apply(event: Event, state: Packed[LState And RState]): (PatternResult[LOut And ROut], Packed[LState And RState]) = {
//
//    val vector = state._1
//    val idx = state._2
//
...
//
//
//case class AliasedParser[Event, InnerState, InnerOut](innerParser: Pattern[Event, InnerState, InnerOut], alias: String)
//  extends Pattern[Event, InnerState, Aliased[InnerOut]] {
//
//  val curriedAlias: InnerOut => Aliased[InnerOut] = (Aliased.apply[InnerOut] _).curried(alias)
//
//  override def apply(event: Event, state: InnerState): (PatternResult[Aliased[InnerOut]], InnerState) = {
...
//
//
//case class MapWithEventParser[Event, State, In, Out](phaseParser: Pattern[Event, State, In], f: Event => In => Out) extends Pattern[Event, State, Out] {
//
...
//}
//
//case class MapParser[Event, State, In, Out](phaseParser: Pattern[Event, State, In], f: In => Out) extends Pattern[Event, State, Out] {
//
//  override def apply(event: Event, oldState: State): (PatternResult[Out], State) = {
//    val (phaseResult, state) = phaseParser.apply(event, oldState)
...
//}
//
//trait VectorStatefulParser[Event, State, Out] extends Pattern[Event, State, Out]{
//
//  override def size = 1
//}
//
...
//
//  override def apply(v1: Event, v2: Packed[NoState]): (PatternResult[T], NoState) = Success(extract(v1)) -> ()
//

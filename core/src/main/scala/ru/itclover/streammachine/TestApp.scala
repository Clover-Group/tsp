...
import ru.itclover.streammachine.core.{AndParser, PhaseParser, TimeInterval, Window}
...
//      )
  ...
  val stateMachine = StateMachineMapper(phase)

  val collector = events.foldLeft(stateMachine) { case (mapper, event) => mapper(event) }

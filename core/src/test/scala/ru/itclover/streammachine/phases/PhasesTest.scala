...
import ru.itclover.streammachine.core.AggregatingPhaseParser.deriv
...
class PhasesTest extends FunSuite with Matchers {
  ...
  test("Derivation phase should work") {
    ...
    val (result1, state1) = deriv(speed).apply(Event(100, Instant.now()), initialState)
    val (result2, state2) = deriv(speed).apply(Event(200, Instant.now()), state1)
